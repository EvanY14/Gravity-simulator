...
class Particle:
    ...
    def get_acceleration(self, p):
        ...
        self.acceleration[0] = (sum_x / self.get_mass()) / 2.5
        self.acceleration[1] = (sum_y / self.get_mass()) / 2.5
        return None
        ...
        for p in particles:
            if not p == self:
                if self.get_distance_to_particle(p) <= self.get_radius() + p.get_radius():
                    ...
                    # if not self.get_x() == p.get_x() and not self.get_y() == p.get_y() and not p.get_mass() == 0:
                        # sum_x += -1 * sign_change_x * (self.gravitational_constant * (
                        #         (self.get_mass() * p.get_mass()) / pow(self.get_x() - p.get_x(), 1)))
                        if not (self.get_x() == p.get_x() and self.get_y() == p.get_y()):
                            sum_x += ((self.gravitational_constant * (p.get_mass() / self.get_mass())) / pow(
                                pow(self.get_x() - p.get_x(), 2) + pow(self.get_y() - p.get_y(), 2), 1.5)) / (p.get_x() - self.get_x())
